...
- Implementers SHOULD use `urn:uuid:0` syntax for IDs and references.
    - Implementers SHOULD populate `Bundle.entry.fullUrl` elements with short resource-scheme URIs (e.g., `{"fullUrl": "urn:uuid:0}`).
    - Implementers SHOULD populate `Reference.reference` elements with short resource-scheme URIs (e.g., `{"patient": {"reference": "urn:uuid:0"}}`) which SHALL resolve within the bundle.
