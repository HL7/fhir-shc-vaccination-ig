...
- Implementers SHOULD use `resource:0` syntax for IDs and references.
    ...
    - _Note that the Bundle examples reflect this guidance for their contained resources, but the other resource-specific examples cannot because the FHIR IG Publisher requires their `id` value matches their filename.
    Additionally, the FHIR validator currently does not support the resource-scheme URIs. For validation, the uri:urn scheme can be used._
