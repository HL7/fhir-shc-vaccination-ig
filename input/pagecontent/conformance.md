...
2. The FHIR terminology server at <tx.fhir.org> does not yet support AIR, ATC, GTIN, and ICD-11 codes, which causes a possible error like `The code provided (http://id.who.int/icd11/mms#XM0GQ8) is not in the value set http://hl7.org/fhir/uv/smarthealthcards-vaccination/ValueSet/vaccine-target-icd-11, and a code from this value set is required: The code system "http://id.who.int/icd11/mms" is not known (encountered paired with code = "XM0GQ8"); The code provided (http://id.who.int/icd11/mms#XM0GQ8) is not valid in the value set 'VaccineTargetICD11' (from http://tx.fhir.org/r4)`. We encourage Issuers and Verifiers to use CVX or SNOMED until the terminology server is updated.
